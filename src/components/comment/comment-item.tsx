...
export function CommentItem({ comment, sideHustleId, onRefresh }: CommentItemProps) {
  ...
  return (
    <div className="group">
      <div className="rounded-lg border bg-card/50 p-4 transition-colors hover:bg-card/80">
        <div className="flex gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <Button
              variant="ghost"
              size="sm"
              className="h-7 text-xs gap-1 text-muted-foreground hover:text-foreground transition-colors"
            ...
            {showReply && (
              <div className="mt-3 ml-2 pl-3 border-l-2 border-primary/30 bg-muted/20 rounded-r-lg py-2">
                <CommentForm
      ...
      {comment.replies && comment.replies.length > 0 && (
        <div className="ml-6 mt-2 space-y-2 border-l-2 border-primary/20 pl-4">
          {(showAllReplies ? comment.replies : comment.replies.slice(0, 3)).map((reply) => (
            <div key={reply.id} className="flex gap-2 bg-muted/20 rounded-lg p-3">
          ...
          {comment.replies.length > 3 && !showAllReplies && (
            <Button
              variant="ghost"
              size="sm"
              className="h-7 text-xs gap-1 text-muted-foreground hover:text-primary transition-colors"
